...
const SummaryCard: React.FC<SummaryCardProps> = ({ children, title, values,className }) => {
  return (
    <div className={`bg-white shadow rounded p-4 min-w-[200px] w-full  ${className} `}>
      <div className="flex items-center gap-2 mb-3 text-lg font-semibold py-2 border-b-2 border-brandPrimary">
        {children}
        <span>{title}</span>
      </div>
      <div className="space-y-1 flex md: flex-row flex-co items-center gap-3 justify-between">
        {values?.map((item, i) => (
          <div key={i} className="">
