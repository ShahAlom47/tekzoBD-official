...
const UserOrderCard: React.FC<OrderCardProps> = ({ order }) => {
  const { _id, meta, cartProducts, pricing, paymentInfo, shippingInfo } = order;
    ...
    try {
      ...
      if (response?.success) {
        ...
        await sendNewNotification({
          ...
          message: `${shippingInfo?.name || "A customer"} cancelled order ${
            _id || ""
          } worth ${pricing?.grandTotal?.toFixed(2) || "N/A"} BDT on ${new Date(
            meta?.checkoutAt || ""
          ).toLocaleString()}.`,
  ...
  return (
    <div className="rounded-xl shadow-lg border border-gray-300 p-6 bg-white hover:shadow-xl transition-shadow duration-300 h-full flex flex-col justify-between">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
        ...
        >
          {meta?.orderStatus.charAt(0).toUpperCase() +
            meta?.orderStatus.slice(1)}
        </span>
      </div>
        ...
        <div className="mb-6">
          ...
          <ul className="divide-y divide-gray-200 max-h-48 overflow-y-auto">
            {cartProducts.map((item, idx) => (
              ...
              >
                <span className="truncate max-w-[65%]">{item.productName}</span>
      ...
      <div className="border-t border-gray-200 pt-4 flex flex-col sm:flex-row sm:justify-between sm:items-center text-sm text-gray-700 space-y-2 sm:space-y-0">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          ...
          <div>
            ...
            {shippingInfo?.deliveryMethod ?? "N/A"}
          </div>
